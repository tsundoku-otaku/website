...
If you like to download and organize your media, then you want to know how to manage your own series in Tsundoku.
...
1. In the location you specified as your storage location (e.g., `/Tsundoku/`), there should be a `local` folder. Place correctly structured series inside that (e.g. `/Tsundoku/local/`).
...
Note that a single folder or archive file is treated as a single chapter. For example, Tsundoku will not automatically split an `EPUB` file containing multiple chapters into separate chapters within the app.

### Folder structure

Tsundoku requires a specific folder structure for local series to be correctly processed.
...
Tsundoku will see three chapters in a single series.
...
However, the filename format is not required to be the same; for example, Tsundoku adds a disambiguating hash based on the chapter URL when downloading chapters, but you do not need to do this.

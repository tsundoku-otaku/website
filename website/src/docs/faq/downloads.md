...
Note that Tsundoku will download from up to five different sources in parallel.

## Why did my downloads stop midway?
Downloads stopping midway may be related to network connection issues or source problems.
**Tsundoku** will provide notifications regarding encountered errors during download attempts.
...
**Tsundoku** typically prevents series pages in downloads from appearing in your device's photo gallery by default through a `.nomedia` file.
...
Because of the prevalence of operating systems like Windows which have arbitrary limitations on special characters in filenames, by default Tsundoku will avoid using certain characters in filenames, specifically: `"*:<>?\|`.
...
If you must use Tsundoku with such an operating system, you can enable the option in the advanced settings for "Disallow non-English filenames", which will prevent any non-English (non-ASCII) characters from being used in filenames.
...
None of the above considerations affect the way series and chapters are displayed in Tsundoku, which is based on their metadata rather than filenames.
...
If you need to change the "Disallow non-English filenames" option, you may need to do some manual work to ensure that Tsundoku can still find your existing downloads.
Chapter filenames do not need to be changed, as Tsundoku is able to check multiple options for a chapter filename, and will find the already-downloaded chapters.
